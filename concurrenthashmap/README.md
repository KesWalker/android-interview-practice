...
counter, a lazy cache, and a leaderboard, all backed by a single
`ConcurrentHashMap` that many threads hammer at once. Each task is a small
function that's currently unwritten, with a matching test that's **red** -
some of those tests fire hundreds of real threads at the same key to prove
the naive "check, then act" approach doesn't survive. Make each test
**green**, one at a time, and you'll have written the atomic-update idioms
